...
function get_services()
	oldServ = read_from_cache(urlCrest, Inf64)
...
function resolve_item_prices(types)
	...
	for typeID in missing
		...
		sellPrice = reduce(min, Inf64, map(o->o["price"], sell))
		if sellPrice == Inf64
			info("Warning: Cannot find price for typeID $typeID - $(itemTypes[typeID]["name"])")
		end
...
function resolve_item_prices_evec(types)
	global itemPrices
	itemPricesFile = joinpath(cacheDir, "itemPrices.json")
	if itemPrices == nothing
		itemPrices = json_read(itemPricesFile)
		if itemPrices == nothing
			itemPrices = Dict{Int, Any}()
		end
	end
	...
	json_write(itemPricesFile, itemPrices)
end
...
get_blueprints() = get_or_import(joinpath(cacheDir, "blueprints.json"), import_blueprints)
